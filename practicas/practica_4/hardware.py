...
STADISTICS_INTERRUPTION_TYPE = "#STAT"
...
class Clock():
    ...
    def __init__(self, manualExecution):
        self._subscribers = []
        self._running = False
        self._manualExecution = manualExecution
        self._tickNbr = 0
    ...
    def start(self):
        ...
        if(not self._manualExecution):
            t = Thread(target=self.__start)
            t.start()

    def __start(self):
        while (self._running):
            self.tick(self._tickNbr)
            self._tickNbr += 1

    def tick(self, tick):
        log.logger.info("        --------------- tick: {tickNbr} ---------------".format(tickNbr = tick))
...
class Cpu():
    ...
    def _execute(self, ticknbr):
        self._interruptVector.handle(IRQ(STADISTICS_INTERRUPTION_TYPE, {"pc":self.pc - 1, "tick":ticknbr}))
        if ASM.isEXIT(self._ir):
            ...
            log.logger.info("cpu - Exec: {instr}, PC={pc}".format(instr=self._ir, pc=self.pc-1))
...
class Timer:
    ...
    def tick(self, tickNbr):
        ...
        if self._active and self._cpu.isBusy():
            self._tickCount += 1
            log.logger.info("Timer after: {tickCount}".format(tickCount=self._tickCount))
...
class Hardware():
    ...
    ## Setup our hardware
    def setup(self, memorySize=256, manualClock=False):
        ## add the components to the "motherboard"
        self._memory = Memory(memorySize)
        self._interruptVector = InterruptVector()
        self._clock = Clock(manualClock)
